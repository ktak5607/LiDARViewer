...
int main()
{
	...
	vector<unsigned char> fileData(4);
	f.read((char*)&fileData[0], 4);
	
	unsigned short sourceID = 0;
	f.read(reinterpret_cast<char*>(&sourceID), sizeof(sourceID));
	cout << sourceID << endl;
	
	
	unsigned short globalEncoding = 0;
	f.read(reinterpret_cast<char*>(&globalEncoding), sizeof(globalEncoding));
	cout << globalEncoding << endl;

	unsigned long projID1 = 0;
	f.read(reinterpret_cast<char*>(&projID1), sizeof(projID1));

	unsigned short projID2 = 0;
	f.read(reinterpret_cast<char*>(&projID2), sizeof(projID2));

	unsigned short projID3 = 0;
	f.read(reinterpret_cast<char*>(&projID3), sizeof(projID3));

	vector <unsigned char> projID4(8);
	f.read((char*)&projID4[0], 8);
	
	unsigned char majorVersion = 'a';
	f.read((char*)&majorVersion, 1);
	cout << (int)majorVersion << endl;

	unsigned char minorVersion = 'a';
	f.read((char*)&minorVersion, 1);
	cout << (int)minorVersion << endl;

	vector <char> sysIdentifier(32);
	f.read((char*)&sysIdentifier[0], 32);
	
	
	vector <char> genSoft(32);
	f.read((char*)&genSoft[0], 32);
	
	unsigned short dayCreated = 0;
	f.read(reinterpret_cast<char*>(&dayCreated), sizeof(dayCreated));
	cout << dayCreated << endl;

	unsigned short yearCreated = 0;
	f.read(reinterpret_cast<char*>(&yearCreated), sizeof(yearCreated));
	...
	f.read(reinterpret_cast<char*>(&offsetToPointData), sizeof(offsetToPointData));
	

	unsigned long numVarLenRec = 0;
	f.read(reinterpret_cast<char*>(&numVarLenRec), sizeof(numVarLenRec));

	unsigned char pntFormat;
	f.read((char*)&pntFormat, 1);
	cout << (int)pntFormat << endl;

	unsigned short pntDataLen = 0;
	f.read(reinterpret_cast<char*>(&pntDataLen), sizeof(pntDataLen));

	unsigned long numOfPnts = 0;
	f.read(reinterpret_cast<char*>(&numOfPnts), sizeof(numOfPnts));

	unsigned long numOfPntsRet[5];
	f.read(reinterpret_cast<char*>(&numOfPntsRet), sizeof(numOfPntsRet));

	double xScaleFactor = 0.0;
	f.read(reinterpret_cast<char*>(&xScaleFactor), sizeof(xScaleFactor));
	double yScaleFactor = 0.0;
	f.read(reinterpret_cast<char*>(&yScaleFactor), sizeof(yScaleFactor));
	double zScaleFactor = 0.0;
	f.read(reinterpret_cast<char*>(&zScaleFactor), sizeof(zScaleFactor));

	double xOffset = 0.0;
	f.read(reinterpret_cast<char*>(&xOffset), sizeof(xOffset));
	double yOffset = 0.0;
	f.read(reinterpret_cast<char*>(&yOffset), sizeof(yOffset));
	double zOffset = 0.0;
	f.read(reinterpret_cast<char*>(&zOffset), sizeof(zOffset));

	f.seekg(832, ios_base::beg);
	long xBaseTest = 0;
	f.read(reinterpret_cast<char*>(&xBaseTest), sizeof(xBaseTest));
	double x = (xBaseTest * xScaleFactor) + xOffset;
	cout << x << endl;
	f.seekg(832, ios_base::beg);
